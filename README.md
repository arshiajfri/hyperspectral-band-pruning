...
This project provides an object-oriented Python framework for analyzing **hyperspectral band-to-band correlation**. The primary goal is to identify and remove redundant spectral bands by applying a correlation threshold, effectively performing dimensionality reduction while preserving essential spectral information.

This approach is crucial for preparing hyperspectral data for machine learning models.
...
│   ├── analyzer.py                 # The HyperspectralAnalyzer class
...
Modify the settings in `src/config.py` to match your data and parameters.
...
CONFIG = {
    ...
    # --- Analysis Parameters ---
    # Threshold for removing bands. 0.95 is aggressive. 0.99 is conservative.
...
The script will automatically create the `results/` directory and save all output plots there.

### Accessing Results Programmatically

You can also import and use the `HyperspectralAnalyzer` class in your own scripts or notebooks.

```python
from src.analyzer import HyperspectralAnalyzer
